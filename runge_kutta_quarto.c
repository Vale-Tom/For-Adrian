...
int main(){
  ...
  double x0, v0, omega2, x, v, t, dt, k, m, xv, energia0, energiat, delta_energia,y1,y2,y3,y4;
  int  n;
  FILE *fp;
  FILE *fq;
  FILE *fr;   
 ...
 for(int i=1; i<=n; i++)
  {
    ...
    y1=v*dt;
    y2=(omega2*(x+y1/2)*(dt/2))*dt;
    y3=(omega2*(x+y2/2)*(dt/2))*dt;
    y4=(omega2*(x+y3)*(dt))*dt;
